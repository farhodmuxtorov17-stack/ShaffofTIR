...
class IsAdminOrInstructor(BasePermission):
    """Allow access to SUPER_ADMIN or INSTRUCTOR."""
    message = "Доступ разрешён администраторам и инструкторам."

    def has_permission(self, request, view):
        return bool(
            request.user
            and request.user.is_authenticated
            and request.user.role in ("SUPER_ADMIN", "INSTRUCTOR")
        )


class IsAdminOrTechSpec(BasePermission):
    """Allow access to SUPER_ADMIN or TECHSPEC."""
    message = "Доступ разрешён администраторам и техническим специалистам."

    def has_permission(self, request, view):
        return bool(
            request.user
            and request.user.is_authenticated
            and request.user.role in ("SUPER_ADMIN", "TECHSPEC")
        )
