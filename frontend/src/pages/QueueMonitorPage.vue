...
async function completeCurrent() {
  ...
  try {
    ...
    // 2. Run shot analysis on the completed entry
    if (currentEntry.value.id) {
      try {
        ...
        toast.success(
          `Анализ: ${analysis.hit_count}/${analysis.total_shots_detected} попаданий, точность ${analysis.accuracy}%`
        )
      } catch (e: any) {
        toast.warning('Анализ не удался, результаты сохранены вручную')
      }
    }
  ...
  socket = useQueueSocket(queueId, (event) => {
    switch (event.type) {
      ...
      case 'queue_completed':
        ...
        break
      case 'analysis_ready':
        toast.success(`Анализ: ${event.data.hit_count} попаданий, точность ${event.data.accuracy}%`)
        break
    }
...
<template>
  <div class="space-y-6">
    ...
    <div v-if="queue" class="flex gap-3 flex-wrap">
      ...
      >
        <span v-if="analysisRunning">⏳ Анализ...</span>
