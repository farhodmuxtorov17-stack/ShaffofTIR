...
async function doLogin() {
  ...
  try {
    ...
    else if (user.role === 'SUPER_ADMIN') router.push('/sessions')
    else if (user.role === 'MANAGER') router.push('/sessions')
    else { error.value = 'Недостаточно прав для входа'; authState.user = null; localStorage.clear() }
  } catch (e: any) {
