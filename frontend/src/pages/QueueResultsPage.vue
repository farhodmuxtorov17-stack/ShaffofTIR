...
async function loadResults() {
  ...
  try {
    ...
    if (found) {
      ...
      toast.warning('Результаты Анализа не найдены')
    }
  } catch (e: any) {
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold text-slate-100">Результаты Анализа</h1>
    ...
    <div v-else-if="analysis" class="space-y-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <span class="text-slate-400 text-sm">Уверенность: {{ Math.round(analysis.confidence * 100) }}%</span>
      </div>
      ...
      <div class="grid md:grid-cols-2 gap-4">
        ...
        <div class="bg-slate-800/50 rounded-xl p-4 border border-slate-700">
          <div class="text-sm text-slate-400 mb-2">Фото после стрельбы (разметка)</div>
