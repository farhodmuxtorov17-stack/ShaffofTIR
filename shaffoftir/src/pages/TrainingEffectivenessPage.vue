...
<template>
  <div class="space-y-6">
    <div>
      ...
      <p class="text-sm text-gray-500 mt-1">{ isUz ? 'Mashg\u02bbulot natijalari tahlili' : 'Анализ результатов тренировок' }</p>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <div class="card"><p class="text-xs text-gray-400 uppercase mb-1">{{ isUz ? 'Jarayonda' : 'В процессе' }}</p><p class="text-2xl font-bold text-amber-600">{{ masterStore.trainingAssignments.filter(a => a.status === 'IN_PROGRESS').length }}</p></div>
      <div class="card"><p class="text-xs text-gray-400 uppercase mb-1">{{ isUz ? 'Muddati o\u02bbtgan' : 'Просрочено' }}</p><p class="text-2xl font-bold text-red-500">{{ masterStore.trainingAssignments.filter(a => a.status === 'OVERDUE').length }}</p></div>
      <div class="card"><p class="text-xs text-gray-400 uppercase mb-1">{{ isUz ? 'Tayinlangan' : 'Назначено' }}</p><p class="text-2xl font-bold text-gray-700">{{ masterStore.trainingAssignments.filter(a => a.status === 'ASSIGNED').length }}</p></div>
    </div>

    <div class="card">
      <h2 class="text-sm font-bold text-gray-700 mb-4">{{ isUz ? 'Rejalar bo\u02bbyicha progress' : 'Прогресс по планам' }}</h2>
      <div class="space-y-3">
    ...
    <div class="card">
      <h2 class="text-sm font-bold text-gray-700 mb-4">{{ isUz ? 'Eng yaxshi otuvchilar' : 'Топ стрелков' }}</h2>
      <div class="space-y-2">
