  ...
  <div class="space-y-6">
    ...
    <div class="card overflow-hidden p-0">
      <table class="w-full text-left text-sm">
        <thead class="bg-gray-50/70 border-b border-shell-border text-gray-500"><tr><th class="px-4 py-3 font-medium">{{ isUz ? 'Xodim' : 'Сотрудник' }}</th><th class="px-4 py-3 font-medium">{{ isUz ? 'Unvon' : 'Звание' }}</th><th class="px-4 py-3 font-medium">{{ isUz ? 'Daraja' : 'Уровень' }}</th><th class="px-4 py-3 font-medium">{{ isUz ? 'Ruxsat' : 'Допуск' }}</th><th class="px-4 py-3 font-medium">{{ isUz ? 'Sana' : 'Дата' }}</th><th class="px-4 py-3 font-medium">{{ isUz ? 'Aniqlik' : 'Точность' }}</th></tr></thead>
        <tbody class="divide-y divide-shell-border">
          <tr v-for="c in certifications" :key="c.employee.id" class="hover:bg-gray-50/50 transition">
            ...
            <td class="px-4 py-3"><span class="badge-success">{{ isUz ? 'Ruxsat berilgan' : 'Допущен' }}</span></td>
