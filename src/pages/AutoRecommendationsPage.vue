    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div v-for="rec in recommendations" :key="rec.id" class="card p-4 border-l-4" :class="rec.priority === 'HIGH' ? 'border-l-red-500' : rec.priority === 'MEDIUM' ? 'border-l-yellow-500' : 'border-l-blue-500'">
        ...
        <div class="flex items-center justify-between">
          ...
          <button class="text-xs font-medium text-green-600 hover:text-green-700">{{ isUz ? "Qo\x02BBllash →" : "Применить →" }}</button>
        </div>
      </div>
    </div>
  </div>
...
const recommendations = ref([
  { id: 1, title_ru: 'Низкая точность на 300м', title_uz: '300m da past aniqlik', employee: 'Каримов А.У.', priority: 'HIGH', confidence: 92, desc_ru: 'За последние 5 сессий точность на дистанции 300м снизилась на 18%. Рекомендуется дополнительная тренировка по силуэтным мишеням.', desc_uz: 'So\x02BBnggi 5 sessiyada 300m aniqlik 18% ga tushdi. Siluet nishonlarga qo\x02BBshimcha mashq tavsiya etiladi.' },
  { id: 2, title_ru: 'Перераспределение оружия', title_uz: 'Qurolni qayta taqsimlash', employee: 'Алиев Б.У.', priority: 'MEDIUM', confidence: 78, desc_ru: 'АКС-74У даёт 3 осечки за 50 выстрелов. Рекомендуется ТО или замена на АК-74.', desc_uz: 'AKS-74U 50 otishda 3 o\x02BBt olmadi. TO yoki AK-74 ga almashtirish tavsiya etiladi.' },
  { id: 3, title_ru: 'ТБ-тест не пройден', title_uz: 'TB testi o\x02BBtmagan', employee: 'Хасанов О.Р.', priority: 'HIGH', confidence: 100, desc_ru: 'Сотрудник не прошёл ТБ-тест. Доступ к полигону заблокирован. Требуется повторное прохождение.', desc_uz: 'Xodim TB testidan o\x02BBtmadi. Poligonga kirish bloklangan. Qayta topshirish kerak.' },
